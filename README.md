...
| Internationalization | `next-intl` (locales: `pt` default, `en`) |
| Deploy | Vercel |
...
`/pt` (default locale); switch to `/en` from the language button in the
header, or open [http://localhost:3000/en](http://localhost:3000/en)
directly.
...
This project needs no environment variables or config files to run locally.
`next.config.ts` is a stock, empty Next.js config, there's no `.env`/
`.env.example` committed (`.env*` is gitignored), and there's no
`process.env` read anywhere in `src`. The one external integration, [Vercel
Analytics](https://vercel.com/docs/analytics) (`@vercel/analytics`, wired in
`src/app/[locale]/layout.tsx`), needs no API key or config either — it
activates automatically once the project is deployed on Vercel and is a
silent no-op when run locally. See
[docs/ARQUITETURA.md § Integration surface](docs/ARQUITETURA.md#integration-surface)
for how it's wired.

## Tests

There is no automated test suite in this repository (no `*.test.*`/
`*.spec.*` files, no test runner in `package.json`). The only automated
check is `npm run lint` (ESLint). `TODO: confirmar` whether a test suite is
planned — until then, verify changes manually with `npm run dev`, checking
both locales (`/pt`, `/en`) and both themes.

## Folder structure

```
src/
  proxy.ts             # next-intl middleware: resolves locale, redirects to /pt or /en
  i18n/
    routing.ts          # Locales (pt default, en) and routing config
    request.ts           # Loads messages/{locale}.json server-side
    navigation.ts         # Locale-aware Link/usePathname/useRouter
  app/
    [locale]/
      layout.tsx          # RootLayout: fonts, locale resolution, NextIntlClientProvider + ThemeProvider, fixed Header/Footer
      page.tsx             # Home: Hero + Manifesto + BentoGrid + Setores
      globals.css          # Design tokens (colors, fonts) via Tailwind v4's @theme
  components/
    layout/
      header.tsx        # Fixed nav, conditional dark/light logo, language switcher, mobile menu
    ...
    ui/
      ...
      language-switcher.tsx       # PT/EN toggle button
      mind-logo.tsx               # "mind" product's SVG wordmark
      forge-logo.tsx               # Forge Skill Library's SVG wordmark
      logo-*.tsx                  # Alternative Café Labs logo concepts (not currently used — see docs/ARQUITETURA.md)
  providers/
    theme-provider.tsx  # next-themes wrapper
messages/
  en.json / pt.json     # All UI strings, one namespace per component
public/
